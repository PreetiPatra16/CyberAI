...
import { getModule, getModuleLesson, getModuleQuestions } from "@/lib/content";
...
export default async function ModulePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const lesson = getModuleLesson(slug);
  ...
  return (
    <AppShell>
      ...
      <section className="mt-7 grid gap-5 lg:grid-cols-[1fr_300px]">
        <div className="space-y-5">{lesson.map((section, index) => <article key={section.title} className="card p-6 sm:p-7"><div className="flex gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-black" style={{ background: learningModule.softAccent, color: learningModule.accent }}>{index + 1}</span><div><h2 className="text-xl font-black">{section.title}</h2>{section.body && <p className="mt-3 leading-7" style={{ color: "var(--muted)" }}>{section.body}</p>}{section.bullets && <ul className="mt-3 space-y-3">{section.bullets.map((bullet) => <li key={bullet} className="flex gap-3 leading-6" style={{ color: "var(--muted)" }}><icons.CheckCircle2 className="mt-0.5 shrink-0" size={19} color={learningModule.accent} />{bullet}</li>)}</ul>}</div></div></article>)}</div>
        <aside className="lg:sticky lg:top-7 lg:self-start"><div className="card p-5"><span className="eyebrow">Knowledge check</span><h2 className="mt-2 text-xl font-black">Ready to test your knowledge?</h2><p className="mt-2 text-sm leading-6" style={{ color: "var(--muted)" }}>Answer {questions.length} scenarios, capture your confidence, and score at least {learningModule.passThreshold} points to pass.</p><Link href={`/modules/${slug}/quiz`} className="button-primary mt-5 w-full">Start quiz <span>→</span></Link></div></aside>
      </section>
    </AppShell>
  );
}
