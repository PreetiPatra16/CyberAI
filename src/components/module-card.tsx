...
export function ModuleCard({ module, passed, bestScore }: { module: LearningModule; passed: boolean; bestScore: number }) {
  ...
  return (
    <article className="card overflow-hidden" style={{ borderTop: `6px solid ${module.accent}` }}>
      <div className="p-5">
        ...
        {available ? (
          <>
            <div className="mt-5 rounded-xl p-3" style={{ background: "var(--surface-muted)" }}>
              ...
              <div className="mt-2 h-2 overflow-hidden rounded-full" style={{ background: "var(--border)" }}><div className="h-full rounded-full" style={{ background: module.accent, width: `${Math.min(100, bestScore / module.maxPoints * 100)}%` }} /></div>
            </div>
