...
create function public.submit_quiz_response(
  ...
  question_key text,
  option_key text,
  answer_confidence public.confidence_level
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  where a.id = $1 and a.user_id = auth.uid() and a.status = 'in_progress'
  for update;
  if not found then raise exception 'Active attempt not found'; end if;

  select q.* into v_question
  from public.questions as q
  where q.content_key = $2 and q.module_id = v_attempt.module_id;
  ...
  where qo.content_key = $3 and qo.question_id = v_question.id;
  if not found then raise exception 'Option does not belong to question'; end if;

  select qo.* into v_correct_option
  from public.question_options as qo
  ...
  values(v_attempt.id, v_question.id, v_option.id, $4, v_option.is_correct, v_points_earned);

  select count(*) into v_response_count from public.quiz_responses as qr where qr.attempt_id = v_attempt.id;
  select count(*) into v_question_count from public.questions as q where q.module_id = v_attempt.module_id;

  if v_response_count = v_question_count then
    select coalesce(sum(qr.points_earned), 0) into v_total_score from public.quiz_responses as qr where qr.attempt_id = v_attempt.id;
    select v_total_score >= m.pass_threshold into v_passed from public.modules as m where m.id = v_attempt.module_id;
    update public.quiz_attempts as a set status = 'completed', score = v_total_score, passed = v_passed, completed_at = now() where a.id = v_attempt.id;
    insert into public.module_progress(user_id, module_id, attempt_count, best_score, passed, completed_at)
    values(auth.uid(), v_attempt.module_id, 1, v_total_score, v_passed, case when v_passed then now() end)
    ...
    set attempt_count = public.module_progress.attempt_count + 1,
        ...
        completed_at = case when public.module_progress.passed or excluded.passed then coalesce(public.module_progress.completed_at, now()) end;
    if v_passed then
      insert into public.earned_badges(user_id, module_id) values(auth.uid(), v_attempt.module_id) on conflict do nothing;
    end if;
  end if;
